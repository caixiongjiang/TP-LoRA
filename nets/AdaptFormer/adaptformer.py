from nets.AdaptFormer.swin_transformer import swin_tiny_patch4_window7_224 as create_model_T_224
from nets.AdaptFormer.swin_transformer import swin_small_patch4_window7_224 as create_model_S_224
from nets.AdaptFormer.adaptformer_module import AdaptFormer_Adapter, AdaptFormer_Adapter_CNN
...
class Attention_block(nn.Module):
    ...
    def __init__(self, F_g, F_l, F_int):
        ...
        self.adaptformer_adapter_att = AdaptFormer_Adapter_CNN(in_dim=F_g)
        self.scale = 0.1
    ...
    def forward(self, g, x):
        ...
        psi = self.relu(g1 + x1)
        psi = self.psi(psi)
        out = x * psi + self.adaptformer_adapter_att(x) * self.scale
        return out
...
class AdaptFormer(nn.Module):
    ...
    def __init__(self, num_classes=21, pretrained=False, backbone="swin_T_224", embed_dim=96, base=64):
        ...
        # AdaptFormer-CNN
        self.adaptformer_adapter1 = AdaptFormer_Adapter_CNN(in_dim=filters[0])
        self.adaptformer_adapter2 = AdaptFormer_Adapter_CNN(in_dim=filters[1])
        self.adaptformer_adapter3 = AdaptFormer_Adapter_CNN(in_dim=filters[2])
        self.adaptformer_adapter4 = AdaptFormer_Adapter_CNN(in_dim=filters[3])
        self.scale = 0.1
        # frozen
        self.frozen()
    ...
    def forward(self, x):
        ...
        a4 = self.conv4(f4) + self.adaptformer_adapter4(d4) * self.scale

        d3 = self.up3(a4)
        e3 = self.Att3(g=d3, x=self.change2(feat1))
        e3 = self.convert2(e3)
        f3 = torch.concat([d3, e3], dim=1)
        a3 = self.conv3(f3) + self.adaptformer_adapter3(d3) * self.scale

        d2 = self.up2(a3)
        e2 = self.Att2(g=d2, x=x2)
        e2 = self.convert3(e2)
        f2 = torch.concat([d2, e2], dim=1)
        a2 = self.conv2(f2) + self.adaptformer_adapter2(d2) * self.scale

        d1 = self.up1(a2)
        e1 = self.Att1(g=d1, x=x1)
        e1 = self.convert4(e1)
        f1 = torch.concat([d1, e1], dim=1)
        a1 = self.conv1(f1) + self.adaptformer_adapter1(d1) * self.scale
    ...
    def frozen(self):
        ...
        # 开启所有Adapter层参数（Transformer Block + CNN_Adapter + Att_Adapter）
        for module in self.modules():
            if isinstance(module, AdaptFormer_Adapter):
                for param in module.parameters():
                    param.requires_grad = True
            if isinstance(module, AdaptFormer_Adapter_CNN):
                for param in module.parameters():
                    param.requires_grad = True
...
if __name__ == '__main__':
    model = AdaptFormer(num_classes=3 + 1, pretrained=False, backbone="swin_T_224")
