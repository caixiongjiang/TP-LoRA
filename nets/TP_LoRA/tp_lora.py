...
class TP_LoRA(nn.Module):
    """
    embed_dim:96,128,192
    """
    def __init__(self, text_size, dataset, num_classes=21, pretrained=False, backbone="swin_T_224", embed_dim=96, base=64):
        super(TP_LoRA, self).__init__()
        ...
        self.words_vector = get_vector(size=text_size, dataset=dataset)
        ...
        # Attention block
        self.Att4 = Attention_block(F_g=filters[3], F_l=filters[3], F_int=filters[2], size=text_size, dataset=dataset, words_vector=self.words_vector)
        self.Att3 = Attention_block(F_g=filters[2], F_l=filters[2], F_int=filters[1], size=text_size, dataset=dataset, words_vector=self.words_vector)
        self.Att2 = Attention_block(F_g=filters[1], F_l=filters[1], F_int=filters[0], size=text_size, dataset=dataset, words_vector=self.words_vector)
        self.Att1 = Attention_block(F_g=filters[0], F_l=filters[0], F_int=filters[0] // 2, size=text_size, dataset=dataset, words_vector=self.words_vector)
    ...
    def forward(self, x):
        ...
        e4 = self.convert1(e4)
        f4 = torch.concat([d4, e4], dim=1)
        a4 = self.conv4(f4) + self.tp_lora_adapter4(d4)

        d3 = self.up3(a4)
        e3 = self.Att3(g=d3, x=self.change2(feat1))
        e3 = self.convert2(e3)
        f3 = torch.concat([d3, e3], dim=1)
        a3 = self.conv3(f3) + self.tp_lora_adapter3(d3)

        d2 = self.up2(a3)
        e2 = self.Att2(g=d2, x=x2)
        e2 = self.convert3(e2)
        f2 = torch.concat([d2, e2], dim=1)
        a2 = self.conv2(f2) + self.tp_lora_adapter2(d2)

        d1 = self.up1(a2)
        e1 = self.Att1(g=d1, x=x1)
        e1 = self.convert4(e1)
        f1 = torch.concat([d1, e1], dim=1)
        a1 = self.conv1(f1) + self.tp_lora_adapter1(d1)
    ...
    def frozen(self):
        ...
        for module in self.modules():
            if isinstance(module, TP_LoRA_Adapter):
...
if __name__ == '__main__':
    model = TP_LoRA(text_size='TINY', dataset='Orange-Navel', num_classes=3 + 1, pretrained=False, backbone="swin_T_224").to('cuda:0')
