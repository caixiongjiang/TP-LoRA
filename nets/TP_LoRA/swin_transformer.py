...
from nets.TP_LoRA.text_encode import get_vector



def get_location_config():
    cfg = read_config()
    location_in = cfg['MODEL']['LORA_IN_LOCATION']
    location_out = cfg['MODEL']['LORA_OUT_LOCATION']
    return location_in, location_out
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, size, dataset, words_vector, dim, window_size, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0., bool_adapter=True):
        ...
        if bool_adapter:
            ...
            self.tp_lora_v = None
            # LOCATION-IN
            location_in, location_out = get_location_config()
            if location_in == 'ATT':
                self.tp_lora_q = TP_LoRA_Adapter(in_dim=dim, text_vector=words_vector, size=size,
                                                   dataset=dataset)
                self.tp_lora_v = TP_LoRA_Adapter(in_dim=dim, text_vector=words_vector, size=size,
                                                   dataset=dataset)
            elif location_in == 'MLP':
                pass
            else:
                self.tp_lora_q = TP_LoRA_Adapter(in_dim=dim, text_vector=words_vector, size=size,
                                                 dataset=dataset)
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        # TP-LoRA shortcut
        if self.tp_lora_q is not None:
            delta_q = self.tp_lora_q(x).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            delta_v = self.tp_lora_v(x).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, size, dataset, words_vector, dim, num_heads, window_size=7, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, bool_adapter=True):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        if bool_adapter:
            # TP-LoRA
            self.tp_lora_mlp = None
            # LOCATION-IN
            location_in, location_out = get_location_config()
            if location_in == 'ATT':
                pass
            elif location_in == 'MLP':
                self.tp_lora_mlp = TP_LoRA_Adapter(in_dim=dim, text_vector=words_vector, size=size, dataset=dataset)
            else:
                # TP-LoRA-Adapter
                self.tp_lora_mlp = TP_LoRA_Adapter(in_dim=dim,  text_vector=words_vector, size=size, dataset=dataset)
        else:
    ...
    def forward(self, x, attn_mask):
        ...
        # TP-LoRA's FFN
        if self.tp_lora_mlp is not None:
            x = shortcut + self.drop_path(x)
            x = x + self.drop_path(self.mlp(self.norm2(x))) + self.drop_path(self.tp_lora_mlp(self.norm2(x)))
        else:
            x = shortcut + self.drop_path(x)
            x = x + self.drop_path(self.mlp(self.norm2(x)))
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, size, dataset, words_vector, dim, depth, num_heads, window_size,
        ...
        _, location_out = get_location_config()

        # build blocks
        if location_out == "DEEP":
            self.blocks = nn.ModuleList([
                SwinTransformerBlock(
                    ...
                    bool_adapter=True if i == 0 or i == 1 else False)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, size, dataset, patch_size=4, in_chans=3, num_classes=1000,
        ...
        # add words_vector
        self.words_vector = get_vector(size=size, dataset=dataset)
        ...
        for i_layer in range(self.num_layers):
            ...
            layers = BasicLayer(size=size,
                                dataset=dataset,
                                words_vector=self.words_vector,
