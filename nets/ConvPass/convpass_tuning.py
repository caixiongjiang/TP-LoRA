from nets.ConvPass.swin_transformer import swin_tiny_patch4_window7_224 as create_model_T_224
from nets.ConvPass.swin_transformer import swin_small_patch4_window7_224 as create_model_S_224
from nets.ConvPass.convpass import Convpass, Convpass_CNN
...
class Attention_block(nn.Module):
    ...
    def __init__(self, F_g, F_l, F_int):
        ...
        self.conv_pass_att = Convpass_CNN(in_dim=F_g, xavier_init=True)
...
class ConvPass_Tuning(nn.Module):
    ...
    def __init__(self, num_classes=21, pretrained=False, backbone="swin_T_224", embed_dim=96, base=64):
        ...
        # ConvPass-CNN
        self.conv_pass1 = Convpass_CNN(in_dim=filters[0], xavier_init=True)
        self.conv_pass2 = Convpass_CNN(in_dim=filters[1], xavier_init=True)
        self.conv_pass3 = Convpass_CNN(in_dim=filters[2], xavier_init=True)
        self.conv_pass4 = Convpass_CNN(in_dim=filters[3], xavier_init=True)
    ...
    def frozen(self):
        ...
        # 开启所有ConvPass层参数（Transformer Block + CNN_Convpass + Att_Convpass）
        for module in self.modules():
            if isinstance(module, Convpass):
                for param in module.parameters():
                    param.requires_grad = True
            if isinstance(module, Convpass_CNN):
                for param in module.parameters():
                    param.requires_grad = True
...
if __name__ == '__main__':
    model = ConvPass_Tuning(num_classes=3 + 1, pretrained=False, backbone="swin_T_224")
