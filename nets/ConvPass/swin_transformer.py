...
from nets.ConvPass.convpass import Convpass
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
        ...
        # ConvPass
        self.conv_pass_attn = Convpass(in_dim=dim, xavier_init=False)
        self.conv_pass_mlp = Convpass(in_dim=dim, xavier_init=False)
        self.scale = 1
    ...
    def forward(self, x, attn_mask):
        ...
        # ConvPass's shortcut
        shortcut = x + self.drop_path(self.conv_pass_attn(self.norm1(x))) * self.scale
        x = self.norm1(x)
        x = x.view(B, H, W, C)
        ...
        # ConvPass‘s FFN
        x = shortcut + self.drop_path(x)
        x = x + self.drop_path(self.mlp(self.norm2(x))) + self.drop_path(self.conv_pass_mlp(self.norm2(x))) * self.scale
